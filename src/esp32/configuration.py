...
def map(x, in_min, in_max, out_min, out_max):
    return int(out_min + x *(out_max - out_min)/(in_max - in_min))
    ...
    def write_cnf_file(self, hsv_lines):
        ...
        with open(self.cnf_file, 'r') as f:
            ...
            for line in f:
                dict_line = loads(line)
                if dict_line["data"] == "hsv_lines":
                    dict_line["white_hmin"] = hsv_lines["white_hmin"]
                    dict_line["white_hmax"] = hsv_lines["white_hmax"]
                    dict_line["white_smin"] = hsv_lines["white_smin"]
                    dict_line["white_smax"] = hsv_lines["white_smax"]
                    dict_line["white_vmin"] = hsv_lines["white_vmin"]
                    dict_line["white_vmax"] = hsv_lines["white_vmax"]
                    
                    dict_line["orange_hmin"] = hsv_lines["orange_hmin"]
                    dict_line["orange_hmax"] = hsv_lines["orange_hmax"]
                    dict_line["orange_smin"] = hsv_lines["orange_smin"]
                    dict_line["orange_smax"] = hsv_lines["orange_smax"]
                    dict_line["orange_vmin"] = hsv_lines["orange_vmin"]
                    dict_line["orange_vmax"] = hsv_lines["orange_vmax"]
                    
                    dict_line["blue_hmin"] = hsv_lines["blue_hmin"]
                    dict_line["blue_hmax"] = hsv_lines["blue_hmax"]
                    dict_line["blue_smin"] = hsv_lines["blue_smin"]
                    dict_line["blue_smax"] = hsv_lines["blue_smax"]
                    dict_line["blue_vmin"] = hsv_lines["blue_vmin"]
                    dict_line["blue_vmax"] = hsv_lines["blue_vmax"]
                replacement = replacement + str(dict_line) + "\n"
        sleep(0.1)
        # re-write the file with the new values
        with open(self.cnf_file, 'w') as f:
...
class Car:
    ...
    def calibrate_colors(self):
        new_hsv_lines = {"white_hmin":0, "white_hmax":0, "white_smin":0, "white_smax":0, "white_vmin":0, 
                        "white_vmax":0, "orange_hmin":0, "orange_hmax":0, "orange_smin":0, "orange_smax":0, 
                        "orange_vmin":0, "orange_vmax":0, "blue_hmin":0, "blue_hmax":0, "blue_smin":0, 
                        "blue_smax":0, "blue_vmin":0, "blue_vmax":0}
        ...
        for color in colors:
            ...
            while not self.buttons.previous_is_pressed():
                ...
                self.display.show_text_list(txt)
                if self.buttons.hit_is_pressed():
                    hvalues.append(value[0])
                    svalues.append(value[1])
                    vvalues.append(value[2])
                    txt = []                    
                    txt.append("VALUE SAVED")
                    txt.append("FOR " + color)                
                    txt.append(str(value))
                    txt.append("WAIT 2 sec")
                    self.display.show_text_list(txt)
                    sleep(2)
            if color == "ORANGE":
                new_hsv_lines["orange_hmin"] = min(hvalues)
                new_hsv_lines["orange_hmax"] = max(hvalues)
                new_hsv_lines["orange_smin"] = min(svalues)
                new_hsv_lines["orange_smax"] = max(svalues)
                new_hsv_lines["orange_vmin"] = min(vvalues)
                new_hsv_lines["orange_vmax"] = max(vvalues)
            elif color == "BLUE":
                new_hsv_lines["blue_hmin"] = min(hvalues)
                new_hsv_lines["blue_hmax"] = max(hvalues)
                new_hsv_lines["blue_smin"] = min(svalues)
                new_hsv_lines["blue_smax"] = max(svalues)
                new_hsv_lines["blue_vmin"] = min(vvalues)
                new_hsv_lines["blue_vmax"] = max(vvalues)
            elif color == "WHITE":
                new_hsv_lines["white_hmin"] = min(hvalues)
                new_hsv_lines["white_hmax"] = max(hvalues)
                new_hsv_lines["white_smin"] = min(svalues)
                new_hsv_lines["white_smax"] = max(svalues)
                new_hsv_lines["white_vmin"] = min(vvalues)
                new_hsv_lines["white_vmax"] = max(vvalues)
            sleep(1)
        if self.color_sensor.line_calibration_is_valid(new_hsv_lines):
            txt = []                    
            txt.append("W L:" + str(new_hsv_lines["white_hmin"]) + '_' + str(new_hsv_lines["white_smin"]) + '_' + str(new_hsv_lines["white_vmin"]))
            txt.append("W H:" + str(new_hsv_lines["white_hmax"]) + '_' + str(new_hsv_lines["white_smax"]) + '_' + str(new_hsv_lines["white_vmax"]))
            txt.append("O L:" + str(new_hsv_lines["orange_hmin"]) + '_' + str(new_hsv_lines["orange_smin"]) + '_' + str(new_hsv_lines["orange_vmin"]))
            txt.append("O H:" + str(new_hsv_lines["orange_hmax"]) + '_' + str(new_hsv_lines["orange_smax"]) + '_' + str(new_hsv_lines["orange_vmax"]))
            txt.append("B L:" + str(new_hsv_lines["blue_hmin"]) + '_' + str(new_hsv_lines["blue_smin"]) + '_' + str(new_hsv_lines["blue_vmin"]))
            txt.append("B H:" + str(new_hsv_lines["blue_hmax"]) + '_' + str(new_hsv_lines["blue_smax"]) + '_' + str(new_hsv_lines["blue_vmax"]))
            txt.append("CENT:YES/DOWN:NO")
            self.display.show_text_list(txt)
            btn = self.buttons.wait_button(["HIT", "NEXT"])
            if btn == "HIT":
            ...
            txt.append("W L:" + str(new_hsv_lines["white_hmin"], new_hsv_lines["white_smin"], new_hsv_lines["white_vmin"]))
            txt.append("W H:" + str(new_hsv_lines["white_hmax"], new_hsv_lines["white_smax"], new_hsv_lines["white_vmax"]))
            txt.append("O L:" + str(new_hsv_lines["orange_hmin"], new_hsv_lines["orange_smin"], new_hsv_lines["orange_vmin"]))
            txt.append("O H:" + str(new_hsv_lines["orange_hmax"], new_hsv_lines["orange_smax"], new_hsv_lines["orange_vmax"]))
            txt.append("B L:" + str(new_hsv_lines["blue_hmin"], new_hsv_lines["blue_smin"], new_hsv_lines["blue_vmin"]))
            txt.append("B H:" + str(new_hsv_lines["blue_hmax"], new_hsv_lines["blue_smax"], new_hsv_lines["blue_vmax"]))
            txt.append("PRESS CENTER")
            self.display.show_text_list(txt)
            sleep(0.5)
            self.buttons.wait_hit()
